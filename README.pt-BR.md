...
- Migre de 0.3.3+ em [docs/MIGRATION.pt-BR.md](docs/MIGRATION.pt-BR.md) (linha alvo **0.4.1**).
...
- Prefira crates.io com lockfile: `cargo install ssh-cli --locked` (**0.4.1+** no crates.io; evite **0.3.9** para SCP).
...
- Install falha em drift crypto RC: rode com `--locked` ou use a linha **0.4.1+** (russh 0.62.2) (`scripts/verify_install_resolve.sh`).
