...
- Alvo de release / linha de produto: **0.5.1**.
...
6. Testes completos — `cargo test --locked --all-targets` verde (lib + integration + gaps_v037 + gaps_v038 + gaps_v039 + gaps_v040 + gaps_v041 + gaps_v042 + **gaps_v051**).
7. Suites residuais de gaps verdes — todos os testes em `tests/gaps_v038_integration.rs`, `tests/gaps_v039_integration.rs`, `tests/gaps_v040_integration.rs`, `tests/gaps_v041_integration.rs`, `tests/gaps_v042_integration.rs` e `tests/gaps_v051_integration.rs` passam (incluindo testes que não se chamam `gap_*`).
...
13. Commit e tag locais de release (REL-003) — `git status` limpo no commit de release; mensagem HEAD de Release; tag local `vX.Y.Z` (para 0.5.1: `v0.5.1`); sem push remoto sem autorização.
...
17. Docs de product line = versão Cargo (DOC-003) — toda superfície de product line declara **0.5.1**, incluindo:
...
20. Âncoras do CHANGELOG (CHG-001) — `CHANGELOG.md` tem seção `## [0.5.1]` e âncoras de compare/rodapé para 0.5.1 (e 0.4.x / 0.3.9 anteriores conforme necessário).
...
24. DOC-051 / honestidade 0.5.1 — superfícies de product line documentam:
...
- DOC-003: arquivos de product line (incluindo este par de checklists) contêm `0.5.1`.
...
- CHG-001 / REL: seção no CHANGELOG + tag local `v0.5.1` sem push não autorizado.
...
- 0.5.1 / gaps_v051: export TOML padrão, JSON `vps-export`, dual-read, evento secrets-init, guarda include-secrets, CRUD `vps-added`, empty command, import exit 65.
...
- [../tests/gaps_v051_integration.rs](../tests/gaps_v051_integration.rs) — gates residuais 0.5.1 export/schema v3/secrets
...
- [schemas/README.md](schemas/README.md) — índice de schemas (linha de produto 0.5.1)
