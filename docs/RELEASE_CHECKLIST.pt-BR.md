...
- Alvo de release / linha de produto: **0.3.9**.
...
12. Commit e tag locais de release (REL-003) — `git status` limpo no commit de release; mensagem HEAD de Release; tag local `vX.Y.Z` (para 0.3.9: `v0.3.9`); sem push remoto sem autorização.
...
16. Docs de product line = versão Cargo (DOC-003) — toda superfície de product line declara **0.3.9**, incluindo:
...
19. Âncoras do CHANGELOG (CHG-001) — `CHANGELOG.md` tem seção `## [0.3.9]` e âncora de compare/rodapé para 0.3.9 (e 0.3.8 anterior conforme necessário).
...
- DOC-003: arquivos de product line (incluindo este par de checklists) contêm `0.3.9`.
- DENY-002: `deny.toml` tem `yanked = "deny"`, `ignore = []`, política multiple-versions documentada.
- CHG-001 / REL: seção no CHANGELOG + tag local `v0.3.9` sem push não autorizado.
...
- [schemas/README.md](schemas/README.md) — índice de schemas (linha de produto 0.3.9)
