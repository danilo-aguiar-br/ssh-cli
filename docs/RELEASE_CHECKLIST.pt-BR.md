...
5. Testes completos — `cargo test` verde (lib + integration + gaps_v037 + gaps_v038 + gaps_v039 + gaps_v040 + **gaps_v041**).
6. Suites de gaps 1:1 — todos os `gap_*` em `tests/gaps_v038_integration.rs`, `tests/gaps_v039_integration.rs` e `tests/gaps_v040_integration.rs` / `tests/gaps_v041_integration.rs` verdes.
...
12. Commit e tag locais de release (REL-003) — `git status` limpo no commit de release; mensagem HEAD de Release; tag local `vX.Y.Z` (para 0.4.1: `v0.4.1`); sem push remoto sem autorização.
...
16. Docs de product line = versão Cargo (DOC-003) — toda superfície de product line declara **0.4.1**, incluindo:
...
19. Âncoras do CHANGELOG (CHG-001) — `CHANGELOG.md` tem seção `## [0.4.1]` e âncoras de compare/rodapé para 0.4.1 (e 0.4.0 / 0.3.9 anteriores conforme necessário).
...
22. DOC-041 / honestidade AUD-POST (0.4.1) — superfícies de product line e agentes documentam:
...
- DOC-003: arquivos de product line (incluindo este par de checklists) contêm `0.4.1`.
...
- CHG-001 / REL: seção no CHANGELOG + tag local `v0.4.1` sem push não autorizado.
...
- [schemas/README.md](schemas/README.md) — índice de schemas (linha de produto 0.4.1)
