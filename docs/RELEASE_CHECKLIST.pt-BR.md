...
12. Commit e tag locais de release (REL-003) — `git status` limpo no commit de release; mensagem HEAD de Release; tag local `vX.Y.Z` (para 0.4.2: `v0.4.2`); sem push remoto sem autorização.
...
19. Âncoras do CHANGELOG (CHG-001) — `CHANGELOG.md` tem seção `## [0.4.2]` e âncoras de compare/rodapé para 0.4.2 (e 0.4.0 / 0.3.9 anteriores conforme necessário).
...
22. DOC-041 / honestidade AUD-POST (0.4.2) — superfícies de product line e agentes documentam:
...
- DOC-003: arquivos de product line (incluindo este par de checklists) contêm `0.4.2`.
...
- CHG-001 / REL: seção no CHANGELOG + tag local `v0.4.2` sem push não autorizado.
...
- [schemas/README.md](schemas/README.md) — índice de schemas (linha de produto 0.4.2)
