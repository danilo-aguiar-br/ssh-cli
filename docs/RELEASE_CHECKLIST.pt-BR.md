...
- Alvo de release / linha de produto: **0.5.3**.
...
13. Commit e tag locais de release (REL-003) — `git status` limpo no commit de release; mensagem HEAD de Release; tag local `vX.Y.Z` (para 0.5.3: `v0.5.3`); sem push remoto sem autorização.
...
17. Docs de product line = versão Cargo (DOC-003) — toda superfície de product line declara **0.5.3**, incluindo:
...
20. Âncoras do CHANGELOG (CHG-001) — `CHANGELOG.md` tem seção `## [0.5.3]` e âncoras de compare/rodapé para 0.5.3 (e 0.5.2 / 0.4.x / 0.3.9 anteriores conforme necessário).
...
25. DOC-053 / honestidade 0.5.3 — superfícies de product line documentam:
    ...
    - G19 constante nomeada `SFTP_PERM_MASK` (`0o7777`)
...
- DOC-003: arquivos de product line (incluindo este par de checklists) contêm `0.5.3`.
...
- CHG-001 / REL: seção `## [0.5.3]` no CHANGELOG + tag local `v0.5.3` sem push não autorizado.
...
- [schemas/README.md](schemas/README.md) — índice de schemas (linha de produto 0.5.3)
