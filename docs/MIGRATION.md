...
> Move from ssh-cli 0.3.3 (or later) to 0.5.1 without losing multi-host inventory.
...
- SCP wire fix (0.4.0): crates.io 0.3.9 advertised SCP but the protocol was broken. Upgrade to 0.4.0+ (prefer product line 0.5.1) before relying on `scp`.
...
- If you came from 0.4.0: redacted export could show fake empty-password ciphertext; tunnel could emit `ok:true` and still exit 74 — upgrade wrappers and the binary to 0.5.1.
...
- Prefer `--password-stdin` for secrets; password on argv warns on stderr (0.5.1+).
...
- Prefer secrets CLI flags (`--allow-plaintext-secrets`, `--secrets-key-file`, `--use-keyring`) over env.
...
- Current (0.5.1): new writes use schema v3 and English TOML keys; loads dual-read legacy Portuguese key aliases.
...
- Env fallbacks still work: `SSH_CLI_SECRETS_KEY`, `SSH_CLI_SECRETS_KEY_FILE`, keyring, or XDG `secrets.key`.
...
- Default encryption is on; plaintext requires explicit opt-out (`--allow-plaintext-secrets` or deprecated env).
- Default tracing is error; INFO prose is not expected on agent stderr.
- SCP remains file-only by design in 0.4.0+ (still true in 0.5.1; not a temporary limitation).
...
## 0.5.1 wire format (schema v3) — current
...
- Secrets flags preferred: `--allow-plaintext-secrets`, `--secrets-key-file`, `--use-keyring`.
...
- `secrets init --json` → `event: "secrets-init"`; `secrets reencrypt --json` → `event: "secrets-reencrypt"`; auto key may emit `secrets-key-auto-created`.
