...
- Release target / product line: **0.5.3**.
...
13. Local release commit and tag (REL-003) — clean `git status` for release commit; HEAD message is Release; local tag `vX.Y.Z` (for 0.5.3: `v0.5.3`); no remote push unless authorized.
...
17. Product-line docs match Cargo version (DOC-003) — every product-line surface states **0.5.3**, including:
...
20. CHANGELOG anchors (CHG-001) — `CHANGELOG.md` has section `## [0.5.3]` and compare/footer anchors for 0.5.3 (and prior 0.5.2 / 0.4.x / 0.3.9 as needed).
...
25. DOC-053 / 0.5.3 honesty — product-line surfaces document:
    ...
    - G19 `SFTP_PERM_MASK` named constant (`0o7777`)
...
- DOC-003: product-line files (including this checklist pair) contain `0.5.3`.
...
- CHG-001 / REL: CHANGELOG section `## [0.5.3]` + local tag `v0.5.3` without unauthorized push.
...
- [schemas/README.md](schemas/README.md) — schema index (product line 0.5.3)
