...
- Linha de produto documentada aqui: **0.5.0** (GAP-001–014 fechados; residual LOG/JSON/CLI fechado; correção wire AUD-SCP + JSON de agente para scp/tunnel; AUD-POST EXP-001/TUN-002/CLI-005/006/IO-009 fechados).
...
- Instale com `cargo install ssh-cli --locked` (**0.5.0+** no crates.io; evite **0.3.9** para SCP).
- Não dependa do crates.io **0.3.9** para SCP: aquela release anunciava transferência, mas o protocolo wire estava quebrado (arquivos remotos de 0 bytes ou timeouts). Use **0.5.0+**.
