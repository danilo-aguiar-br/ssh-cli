...
## English
- This directory versions machine-readable JSON contracts for ssh-cli stdout payloads (**0.4.0**). Includes `scp-transfer.schema.json` for `scp upload|download` success JSON (regular files only).
...
- Este diretório versiona contratos JSON legíveis por máquina para payloads stdout do ssh-cli (**0.4.0**).
