...
fn parser_accepts_global_timeout() {
    let args =
        CliArgs::try_parse_from(["ssh-cli", "--timeout", "1234", "vps", "list"]).expect("parse");
...
#[test]
fn verbose_count_accepts_vv_and_vvv() {
    // G14: graduated verbosity via ArgAction::Count.
    let a1 = CliArgs::try_parse_from(["ssh-cli", "-v", "vps", "path"]).expect("-v");
    assert_eq!(a1.verbose, 1);
    let a2 = CliArgs::try_parse_from(["ssh-cli", "-vv", "vps", "path"]).expect("-vv");
    assert_eq!(a2.verbose, 2);
    let a3 = CliArgs::try_parse_from(["ssh-cli", "-vvv", "vps", "path"]).expect("-vvv");
...
fn parser_export_output_pathbuf() {
    let args = CliArgs::try_parse_from(["ssh-cli", "vps", "export", "-o", "/tmp/hosts.toml"])
        .expect("export");
    match args.command {
        Command::Vps {
            ...
            assert_eq!(
                output.as_deref(),
                Some(std::path::Path::new("/tmp/hosts.toml"))
            );
        }
        _ => panic!("export"),
    }
...
fn parser_lang_global_accepts_pt_br() {
    let args =
        CliArgs::try_parse_from(["ssh-cli", "--lang", "pt-BR", "locale"]).expect("lang+locale");
...
fn parser_auth_flatten_on_exec() {
    ...
    match args.command {
        Command::Exec { auth, .. } => {
            ...
            assert_eq!(auth.key_path_string().as_deref(), Some("/tmp/id"));
        }
        _ => panic!("exec"),
    }
}
...
fn parser_edit_enable_sudo_flag() {
    let args = CliArgs::try_parse_from(["ssh-cli", "vps", "edit", "prod", "--enable-sudo"])
        .expect("edit enable");
    match args.command {
        Command::Vps {
            action:
...
fn parser_disable_sudo_global() {
    let args = CliArgs::try_parse_from(["ssh-cli", "--disable-sudo", "vps", "path"]).unwrap();
...
fn parser_doctor_probe_ssh() {
    let args = CliArgs::try_parse_from(["ssh-cli", "vps", "doctor", "--probe-ssh"]).unwrap();
    match args.command {
        Command::Vps {
            action: VpsAction::Doctor {
                probe_ssh, hosts, ..
            },
        } => {
            assert!(probe_ssh);
...
fn parser_max_concurrency_global() {
    let args =
        CliArgs::try_parse_from(["ssh-cli", "--max-concurrency", "8", "health-check", "--all"])
            .unwrap();
...
fn parser_exec_hosts_flag() {
    let args = CliArgs::try_parse_from(["ssh-cli", "exec", "--hosts", "a,b", "uptime"]).unwrap();
    match args.command {
        Command::Exec {
            all, hosts, target, ..
        } => {
...
fn parser_all_hosts_conflict() {
    let err = CliArgs::try_parse_from(["ssh-cli", "exec", "--all", "--hosts", "a", "uptime"]);
    assert!(err.is_err());
}
