...
pub fn load_primary_key() -> SshCliResult<(Option<[u8; 32]>, KeySource)> {
        ...
        .unwrap_or(false)
    {
        match read_keyring() {
            ...
            Ok(None) => {}
            Err(e) => {
                tracing::warn!(err = %e, "keyring indisponível; tentando secrets.key");
            }
        }
    }
...
pub fn ensure_key_for_write() -> SshCliResult<(Option<[u8; 32]>, KeySource)> {
    let (existente, source) = load_primary_key()?;
    if existente.is_some() {
        return Ok((existente, source));
    }
...
pub fn deserialize_secret(armazenado: &str) -> SshCliResult<String> {
    if !is_encrypted_blob(armazenado) {
